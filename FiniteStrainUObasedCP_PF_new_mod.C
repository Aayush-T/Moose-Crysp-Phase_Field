...
#include "FiniteStrainUObasedCP_PF_new_mod.h"
...
registerMooseObject("TensorMechanicsApp", FiniteStrainUObasedCP_PF_new_mod);

InputParameters
FiniteStrainUObasedCP_PF_new_mod::validParams()
{
...
FiniteStrainUObasedCP_PF_new_mod::FiniteStrainUObasedCP_PF_new_mod(const InputParameters & parameters)
  : ComputeStressBase(parameters),
    ...
    _pk2_new(declareProperty<RankTwoTensor>("pk2_new")), // 2nd Piola-Kirchoff Stress   AAYUSH TRIVEDI 3rd
...
void
FiniteStrainUObasedCP_PF_new_mod::initQpStatefulProperties()
{
  for (unsigned int i = 0; i < _num_uo_slip_rates; ++i)
  {
  ...
  //AAYUSH TRIVEDI 1st
  _H[_qp] = 0.0;
  _ee[_qp].setToIdentity();
//  _ep[_qp].setToIdentity();
 ...
 */
void
FiniteStrainUObasedCP_PF_new_mod::computeQpStress()
{
  ...
  do
  {
    ...
    if (substep_iter > _max_substep_iter && _err_tol)
      throw MooseException("FiniteStrainUObasedCP_PF_new_mod: Constitutive failure.");
  } while (_err_tol);
...
void
FiniteStrainUObasedCP_PF_new_mod::preSolveQp()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
...
void
FiniteStrainUObasedCP_PF_new_mod::solveQp()
{
  preSolveStatevar();
  solveStatevar();
  if (_err_tol)
...
void
FiniteStrainUObasedCP_PF_new_mod::postSolveQp()
{
  _stress[_qp] = _fe * _pk2[_qp] * _fe.transpose() / _fe.det();
//AAYUSH TRIVEDI 2nd
  //printf("%f",_stress[_qp]);
...
void
FiniteStrainUObasedCP_PF_new_mod::preSolveStatevar()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
...
void
FiniteStrainUObasedCP_PF_new_mod::solveStatevar()
{
  unsigned int iterg;
  bool iter_flag = true;
  ...
  {
#ifdef DEBUG
    mooseWarning("FiniteStrainUObasedCP_PF_new_mod: Hardness Integration error\n");
#endif
    _err_tol = true;
  }
}
...
bool
FiniteStrainUObasedCP_PF_new_mod::isStateVariablesConverged()
{
  Real diff;
...
void
FiniteStrainUObasedCP_PF_new_mod::postSolveStatevar()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
...
void
FiniteStrainUObasedCP_PF_new_mod::preSolveStress()
{
}

void
FiniteStrainUObasedCP_PF_new_mod::solveStress()
{
  unsigned int iter = 0;
  RankTwoTensor dpk2;
  ...
  {
#ifdef DEBUG
    mooseWarning("FiniteStrainUObasedCP_PF_new_mod: Slip increment exceeds tolerance - Element number ",
                 _current_elem->id(),
                 " Gauss point = ",
                 _qp);
#endif
  ...
  while (rnorm > _rtol * rnorm0 && rnorm0 > _abs_tol && iter < _maxiter)
  {
    ...
    {
#ifdef DEBUG
      mooseWarning("FiniteStrainUObasedCP_PF_new_mod: Failed with line search");
#endif
      _err_tol = true;
      return;
    }
  ...
  {
#ifdef DEBUG
    mooseWarning("FiniteStrainUObasedCP_PF_new_mod: Stress Integration error rmax = ",
                 rnorm,
...
void
FiniteStrainUObasedCP_PF_new_mod::postSolveStress()
{
  _fp[_qp] = _fp_inv.inverse();
}

void
FiniteStrainUObasedCP_PF_new_mod::updateSlipSystemResistanceAndStateVariable()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
...
void
FiniteStrainUObasedCP_PF_new_mod::calcResidJacob()
{
  calcResidual();
  if (_err_tol)
    return;
...
void
FiniteStrainUObasedCP_PF_new_mod::getSlipRates()
{
  for (unsigned int i = 0; i < _num_uo_slip_rates; ++i)
  {
...
void
FiniteStrainUObasedCP_PF_new_mod::calcResidual()
{
  RankTwoTensor iden(RankTwoTensor::initIdentity), ce, ce_pk2, eqv_slip_incr, test;

  Real F_pos, F_neg; // AAYUSH TRIVEDI 2nd
  ...
  //_ep[_qp] = _lag_e[_qp] - _ee[_qp];
  //ee *= 0.5;
  //AAYUSH TRIVEDI 2nd
  computeStrainSpectral(F_pos, F_neg, ce);

  //pk2_new = _elasticity_tensor[_qp] * _ee[_qp];
  test = _elasticity_tensor[_qp] * _ee[_qp];
//  printf("pk2_new %e \t test %e\t",pk2_new[], test);
_resid = _pk2[_qp] - _pk2_new[_qp];
//  _resid = _pk2[_qp] - test;
...
_E[_qp] =
    hist_variable * _D[_qp] + F_neg - _pressure[_qp] * _ee[_qp].trace() * _I[_qp];
_dEdc[_qp] =
    hist_variable * _dDdc[_qp] - _pressure[_qp] * _ee[_qp].trace() * _dIdc[_qp];
_d2Ed2c[_qp] = hist_variable * _d2Dd2c[_qp] -
               _pressure[_qp] * _ee[_qp].trace() * _d2Id2c[_qp];
}
//AAYUSH TRIVEDI 2nd
void
FiniteStrainUObasedCP_PF_new_mod::computeStrainSpectral(Real & F_pos, Real & F_neg, RankTwoTensor & ce)
{
...
//  delta is the trace of volumetric part of elastic G-L strain
  ...
  elastic_enrgy_tensor = _elasticity_tensor[_qp] * _ee[_qp];
  elastic_enrgy_tensor = 0.5 * _ee[_qp] * elastic_enrgy_tensor;
  // Doubt

  ap_cpl += elastic_enrgy_tensor.trace();
  ap_cpl = ap_cpl - 0.5 * Kb * (std::pow(delta,2.0));
  //
  ...
  // RankFourTensor Ppos =
  //     _ee[_qp].positiveProjectionEigenDecomposition(eigval, eigvec);
  ...
  //_pk2_new[_qp] = stress0pos * _D[_qp] - _pressure[_qp] * I2 * _I[_qp] + stress0neg;
  if (Je >= 1.0) { // expansion

    pk2_pos = _elasticity_tensor[_qp] * _ee[_qp];
    pk2_neg = 0.0;
  ...
  _pk2_new[_qp] = (_D[_qp] * pk2_pos) + pk2_neg;
...
void
FiniteStrainUObasedCP_PF_new_mod::calcJacobian()
{
...
void
FiniteStrainUObasedCP_PF_new_mod::calcTangentModuli()
{
...
void
FiniteStrainUObasedCP_PF_new_mod::elastoPlasticTangentModuli()
{
...
void
FiniteStrainUObasedCP_PF_new_mod::elasticTangentModuli()
{
...
bool
FiniteStrainUObasedCP_PF_new_mod::lineSearchUpdate(const Real rnorm_prev, const RankTwoTensor dpk2)
{
